...
public static class GeneralUtils
{
  ...
  /// <para>Note that the path is not searched.  <paramref name="command"/> must contain the full pathname
  /// of the file to be executed.</para>
  ...
  /// <para>Note that the path is not searched.  <paramref name="runProcessInfo.Command"/> must contain the full pathname
  /// of the file to be executed.</para>
  /// <para>By default, the process will wait for an infinite amount of time to complete the operation.
  ...
  public static RunProcessOutput RunProcess(RunProcessInfo runProcessInfo)
  {
    ...
    ProcessStartInfo psi =
      new()
      {
        ...
        /* UseShellExecute must be set to false in order to redirect stdout and stderr. */
    ...
    using (var process = Process.Start(psi))
    {
      ...
      /* The DataReceivedEventArgs.Data property is stripped of any '/n' and/or '/r' trailing characters.
         This is documented [0].  It also violates the Principle of Least Surprise [1].

         I don't see any good reason why MS designed it this way [2].  They intentionally
         change the process's output for no good reason, which is generally a sloppy/bad/frustrating design choice.

         Anyways, it's why ApppendLine() is used to accumulate the output and error lines
         emitted by the called process.

           [0] https://learn.microsoft.com/en-us/dotnet/api/system.diagnostics.datareceivedeventargs.data#remarks
           [1] https://en.wikipedia.org/wiki/Principle_of_least_astonishment
           [2] https://source.dot.net/System.Diagnostics.Process/R/0899baecec5401db.html */

      process.OutputDataReceived += (_, e) =>
      {
        if ((e.Data != null) && runProcessInfo.StdOutPredicate(e.Data))
      ...
      {
        if ((e.Data != null) && runProcessInfo.StdErrPredicate(e.Data))
      ...
      if (process.WaitForExit(runProcessInfo.TimeoutInMilliseconds))
      {
        /* This code might look a bit weird - why is WaitForExit() being
           ...
           From the "Remarks" section for WaitForExit() [0]:
           ...
           [0] https://docs.microsoft.com/en-us/dotnet/api/system.diagnostics.process.waitforexit
        */
